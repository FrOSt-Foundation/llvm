...
bool DCPU16DAGToDAGISel::SelectAddr(SDValue N,
  ...
  Base  = (AM.BaseType == DCPU16ISelAddressMode::FrameIndexBase) ?
    CurDAG->getTargetFrameIndex(AM.Base.FrameIndex, TLI->getPointerTy(CurDAG->getDataLayout())) :
    AM.Base.Reg;
...
void DCPU16DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse()) {
      ReplaceUses(Node, CurDAG->SelectNodeTo(Node, DCPU16::ADD16ri, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, SDLoc(Node), MVT::i16)));
      CurDAG->RemoveDeadNode(Node);
      return;
    }

    ReplaceUses(Node, CurDAG->getMachineNode(DCPU16::ADD16ri, SDLoc(Node), MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, SDLoc(Node), MVT::i16)));
    CurDAG->RemoveDeadNode(Node);
    return;
  }
